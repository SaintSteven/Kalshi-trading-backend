...
import httpx
from app.config import KALSHI_BASE_URL, MLB_STRIKEOUT_PREFIX
...
async def collect_mlb_strikeout_markets(target_date: str | None = None) -> tuple[str, list[Market]]:
    ticker_date = kalshi_ticker_date(target_date)
    async with httpx.AsyncClient(headers={"User-Agent": "KalshiTradingPlatform/0.1"}) as client:

...
# Machine-readable slate gate: Kalshi event date must map to the same nflverse season/week as the projection.
...
def event_date(event_ticker):
 hit=re.search(r'-(\d{2}[A-Z]{3}\d{2})',str(event_ticker).upper())
 return datetime.strptime(hit.group(1),'%y%b%d').date() if hit else None
def slate_week(event_ticker):
 dt=event_date(event_ticker)
 q=sched[(sched.gameday==dt) & (pd.to_numeric(sched.season,errors='coerce')==2026)]
...
for x in m.itertuples():
 ...
 if int(r.projection_season)!=2026 or int(r.projection_week)!=int(market_week):
  raise SystemExit(f'Slate consistency gate failed: {x.ticker} market_week={market_week} projection={r.projection_season}-W{r.projection_week}')
...
o=pd.DataFrame(rows)
import os
scope=os.environ.get('NFL_EVENT_FILTER','').strip().upper()
if scope and not o.empty: o=o[o.event_ticker.astype(str).str.upper().str.contains(scope,regex=False)].copy()

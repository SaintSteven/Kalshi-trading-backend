...
UA={'User-Agent':'kalshi-nfl-market-efficiency-v0.03.2-full-validation'}
HOURS=[6,3,1,.5]
...
def candles(series,ticker,start,end,source):
 paths=[]
 if source=='recent_live': paths=[f'/series/{series}/markets/{ticker}/candlesticks',f'/historical/markets/{ticker}/candlesticks']
 else: paths=[f'/historical/markets/{ticker}/candlesticks',f'/series/{series}/markets/{ticker}/candlesticks']
 last=None
 for p in paths:
  try:return get(p,{'start_ts':start,'end_ts':end,'period_interval':1}).get('candlesticks',[]) or []
...
def load_schedule():
 ...
 g=g[g['game_type'].astype(str).eq('REG')].copy() if 'game_type' in g.columns else g
 ...
 g['away_n']=g['away_team'].map(norm_team);g['home_n']=g['home_team'].map(norm_team)
 return g
...
def map_kickoff(event,sched):
 ...
 d,teams=p
 c=sched[sched.gameday.eq(d)]
 for r in c.itertuples(index=False):
  a,h=norm_team(r.away_team),norm_team(r.home_team)
  if teams in (a+h,h+a):return pd.Timestamp(r.kickoff_utc),f'{a}@{h}'
 return None,None
...
def main():
 ...
 out=Path(a.out);out.mkdir(parents=True,exist_ok=True);sched=load_schedule();inv=pd.read_csv(a.inventory)
 ...
 for j,m in enumerate(base.itertuples(index=False),1):
  kickoff,game=map_kickoff(getattr(m,'event_ticker',None),sched)
  ...
  try:cs=candles(m.series_ticker,m.market_ticker,kts-int(7*3600),kts-int(20*60),getattr(m,'source','historical'))
  except Exception as e:errs.append({'ticker':m.market_ticker,'series_ticker':m.series_ticker,'error':repr(e)});continue
  snaps={h:snapshot(cs,kts-int(h*3600)) for h in HOURS};exit30=snaps[.5]
  close=pd.to_datetime(getattr(m,'close_time',None),errors='coerce',utc=True)
  delta_min=(close-kickoff).total_seconds()/60 if pd.notna(close) else None
  ...
  for h,s in snaps.items():
   ...
   for side in ('yes','no'):
    ...
    move=(exit30[f'{side}_bid']-entry) if h>.5 and exit30 else None
    rows.append({'prop_class':a.prop_class,'prop_subtype':subtype,'series_ticker':m.series_ticker,'market_ticker':m.market_ticker,'event_ticker':m.event_ticker,'game':game,'kickoff_utc':kickoff.isoformat(),'season':kickoff.year,'holdout_2025':kickoff.year==2025,'close_time':m.close_time,'close_minus_kickoff_min':delta_min,'participant_key':participant,'thesis_key':thesis,'side':side,'entry_horizon':f'T-{h:g}h','entry_ts':s['ts'],'quote_age_sec':s['age_sec'],'entry_price':entry,'price_bucket':bucket(entry),'settled_win':win,'settlement_pnl':win-entry,'exit_30m_bid':exit30[f'{side}_bid'] if exit30 else None,'movement_pnl_to_30m':move})
  if j%250==0:print(a.prop_class,a.shard_index,j)
 pd.DataFrame(rows).to_csv(out/'executable_ledger.csv',index=False);pd.DataFrame(errs).to_csv(out/'errors.csv',index=False)

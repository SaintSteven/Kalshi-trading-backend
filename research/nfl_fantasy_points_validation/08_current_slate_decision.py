...
for x in m.itertuples():
 ...
 rows.append({'ticker':x.ticker,'player':x.kalshi_player_name,'player_id':r.player_id,'position':r.position,'team':getattr(r,'team',None),'identity_method':identity_method,'projection_fp':proj,'projection_season':int(r.projection_season),'projection_week':int(r.projection_week),'threshold':th,'fair_yes':fair,'yes_bid':bid,'yes_ask':ask,'edge_vs_ask':edge,'decision':decision,'qc':'|'.join(qc_flags) if qc_flags else 'PASS','model':model.get('model_version','NFL-FFPTS-RIDGE-EMPIRICAL-v1')})
o=pd.DataFrame(rows)
...
summary={'pipeline':'NFL FFPTS v1','research_only':True,'orders_placed':False,'frozen_model':model.get('model_version','NFL-FFPTS-RIDGE-EMPIRICAL-v1'),'supported_markets':len(o),'quotes_present':int(o.yes_ask.notna().sum()) if len(o) else 0,'full_name_identity_matches':int((o.identity_method=='full_name').sum()) if len(o) else 0,'fallback_identity_matches':int((o.identity_method=='abbrev_position_fallback').sum()) if len(o) else 0,'team_change_reviews':int(o.qc.str.contains('TEAM_CHANGE_REVIEW').sum()) if len(o) else 0,'PAPER':int((o.decision=='PAPER').sum()) if len(o) else 0,'WATCH':int((o.decision=='WATCH').sum()) if len(o) else 0,'PASS':int((o.decision=='PASS').sum()) if len(o) else 0,'method':'Independent current-slate Ridge point projection plus frozen empirical position residual threshold probability; Kalshi executable ask used only after fair probability is produced.','projection_source':'09_current_slate_projections.csv','identity_gate':'full Kalshi name -> full nflverse display name+position; guarded abbreviated-name+position fallback only when unique','qc_gate':'Frozen Week 1 QB, low-role, team-change, and tail-calibration review rules are applied before PAPER classification.','warning':'Research/paper only. Market prices do not alter model projections or fair probabilities.'}

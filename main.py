...
from app.engines.card_builder import build_market_only_card
from app.engines.market_collector import collect_mlb_strikeout_markets
from app.models import CardRequest, CardResponse, Market

app = FastAPI(title="Kalshi Trading Engine", version="0.1.0")
...
@app.get("/")
async def root() -> dict:
    return {"service": "Kalshi Trading Engine", "version": "0.1.0", "docs": "/docs"}

@app.get("/health")
async def health() -> dict:
    return {"status": "ok", "version": "0.1.0", "time_utc": datetime.now(timezone.utc).isoformat()}

@app.get("/markets", response_model=list[Market])
async def markets(date: str | None = Query(default=None)) -> list[Market]:
    try:
...
async def build_card(request: CardRequest) -> CardResponse:
    ...
    return CardResponse(
        ...
        message="Sprint 1 is working: live markets were collected and normalized. No real picks are produced yet because the independent projection and QC engines are intentionally not connected.",
    )

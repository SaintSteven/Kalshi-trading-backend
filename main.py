...
app=FastAPI(title="Kalshi Trading Engine",version="0.4.0",description="Paper-only two-sided MLB strikeout pricing engine.")
...
@app.get("/")
async def root(): return {"service":"Kalshi Trading Engine","version":"0.4.0","mode":"paper-only","docs":"/docs"}

@app.get("/health")
async def health(): return {"status":"ok","version":"0.4.0","mode":"paper-only","time_utc":datetime.now(timezone.utc).isoformat()}
...
async def markets(date: str|None=None,tradable_only: bool=True,min_ask:int=Query(2,ge=1,le=49),max_ask:int=Query(98,ge=51,le=100),max_combined_ask:int=Query(110,ge=100,le=200)):
    try:
        _,visible,_=await collect_mlb_strikeout_markets(date,tradable_only=tradable_only,min_ask=min_ask,max_ask=max_ask,max_combined_ask=max_combined_ask); return visible
...
async def build_card(request: PaperCardRequest):
    try:
        target_date,markets,_=await collect_mlb_strikeout_markets(request.date,tradable_only=True)
    except ValueError as exc: raise HTTPException(status_code=400,detail="Date must use YYYY-MM-DD.") from exc
    except httpx.HTTPError as exc: raise HTTPException(status_code=502,detail=f"Kalshi market request failed: {exc}") from exc
    recommendations,matched=build_paper_card(markets,request)
    return PaperCardResponse(status="paper_projection_complete",target_date=target_date,markets_reviewed=len(markets),projections_matched=matched,recommendations=recommendations,message="Paper-only projection pricing is active. MODEL EDGE is not a real-money BUY recommendation. Automated pitcher data collection, QC, and backtesting must be completed before live recommendations are enabled.")
